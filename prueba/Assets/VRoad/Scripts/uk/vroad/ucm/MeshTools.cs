...
namespace uk.vroad.ucm
{
    public class MeshTools: ScriptableObject
    {
            ...
            List<TriMesh> tml, float kw, float mw, IMind vm, bool mini)
        {
            ...
            ITaxiZone taxiZoneO = roads[0].GetKerbLane().GetTaxiZone();
            if (bayOI < 0 && taxiZoneO != null) bayOI = taxiZoneO.FirstOccupiedBay();
            if (bayOI < 0) bayOI = 0; // If not chosen yet, and no taxis waiting, draw route from first bay
            double cdo = 0.5 * (taxiZoneO.BayStartDistance(bayOI) + taxiZoneO.BayEndDistance(bayOI));
            float startFirst = (float) (cdo / roads[0].GetKerbLane().Length());
            ...
            List<TriMesh> triMeshList, float kw, float mw, IMind vm, bool mini)
        {
            ...
            float sd1 = (float) stop1.DistanceOnLane();
            float sd2 = (float) stop2.DistanceOnLane();
        ...
        public static TriMesh PyramidTriMesh(Xyz p, float h, float r)
        {
            ...
            return new TriMesh(va, tvia, na, uva, MaterialHint.Stop);
        }
